...
export default function FastFoods(props) {
    const [onlineFastFoods, setOnlineFastFoods] = useState([]);   

    const loadOnlineFastFoods = async () => {
        try{
            let promise = await fetch('https://raw.githubusercontent.com/NewPemika/Delivery/master/FastFoods.json');
            let data = await promise.json();
            console.log("Load Data : " , data);
          //SET STATE
            setOnlineFastFoods(data);
        }catch(error){
