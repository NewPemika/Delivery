...
export default function Noodles(props) {
    const [onlineNoodles, setOnlineNoodles] = useState([]);   

        const loadOnlineNoodles = async () => {
            try{
                let promise = await fetch('https://raw.githubusercontent.com/NewPemika/Delivery/master/NoodlesApi.json');
                let data = await promise.json();
                console.log("Load Data : " , data);
              //SET STATE
                setOnlineNoodles(data);
            }catch(error){
