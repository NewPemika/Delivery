...
export default function RiceBowls(props) {
    const [onlineRiceBowls, setOnlineRiceBowls] = useState([]);   

        const loadOnlineRiceBowls = async () => {
            try{
                let promise = await fetch('https://raw.githubusercontent.com/NewPemika/Delivery/master/RiceBowlsApi.json');
                let data = await promise.json();
                console.log("Load Data : " , data);
              //SET STATE
                setOnlineRiceBowls(data);
            }catch(error){
